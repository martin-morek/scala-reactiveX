...
object Server {
  ...
  val eventParserFlow: Flow[ByteString, Event, NotUsed] =
    reframedFlow.map(s => Event.parse(s))
  ...
  val identityParserSink: Sink[ByteString, Future[Identity]] =
    reframedFlow.map(x => Identity(x.toInt)).toMat(Sink.head)(Keep.right)
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat { () =>
      var expectedSeqNr = 1
      var buffer = ListBuffer.empty[Event]

      event =>
        {
          if (event.sequenceNr <= expectedSeqNr) {
            expectedSeqNr = expectedSeqNr + 1
            buffer = buffer.sortWith((e1, e2) => e1.sequenceNr < e2.sequenceNr)

            def events(b: List[Event]): List[Event] = b match {
              case h :: t if h.sequenceNr <= expectedSeqNr => {
                expectedSeqNr = expectedSeqNr + 1
                buffer -= h
                h :: events(t)
              }
      ...
      event =>
        {
          event match {
            case Follow(_, fromUserId, toUserId) => {
              followers = followers.get(fromUserId) match {
                case Some(f) => followers + (fromUserId -> (f + toUserId))
                case None    => followers + (fromUserId -> Set(toUserId))
              }
              (event, followers) :: Nil
            }
            case Unfollow(_, fromUserId, toUserId) => {
              followers = followers.get(fromUserId) match {
                case Some(f) => followers + (fromUserId -> (f - toUserId))
                case None    => followers - fromUserId
              }
              (event, followers) :: Nil
            }
  ...
  /**
    ...
    */
  def isNotified(userId: Int)(
      eventAndFollowers: (Event, Followers)): Boolean = {
    val event = eventAndFollowers._1
    val followers = eventAndFollowers._2

    event match {
      case Follow(_, _, toUserId) if userId == toUserId     => true
      case Broadcast(_)                                     => true
      case PrivateMsg(_, _, toUserId) if userId == toUserId => true
      case StatusUpdate(_, fromUserId) =>
        followers.getOrElse(userId, Set.empty).contains(fromUserId)
      case _ => false
    }
...
/**
  ...
  */
class Server()(implicit executionContext: ExecutionContext,
  ...
  val eventsFlow: Flow[ByteString, Nothing, NotUsed] =
    Flow
      .fromSinkAndSourceCoupled[ByteString, Nothing](inboundSink, Source.maybe)
...
//    clientIdPromise.future.map(id => actorSystem.log.info("Connected follower: {}", id.userId))
    ...
    val incoming: Sink[ByteString, clientIdPromise.type] =
      identityParserSink.mapMaterializedValue(x =>
        clientIdPromise.completeWith(x))

    val outgoing = Source.fromFutureSource(clientIdPromise.future.map {
      identity =>
